...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: `Pediatricians in ${regionData.label}${pageSuffix}`,
    description: `Find trusted pediatricians in ${regionData.label}, ${regionData.province}. Browse verified listings on ${verticalConfig.name}.`,
...
export default async function RegionPage({ params, searchParams }: Props) {
  ...
  // "No pediatricians in {X} yet. Check back soon!" at HTTP 200, indexable and name-bearing,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <h1 className="text-3xl font-bold mb-2 text-gray-900">
        Pediatricians in {regionData.label}
      </h1>
      <p className="text-gray-600 mb-8">
        Browse {total.toLocaleString("en-US")} {total === 1 ? "pediatrician" : "pediatricians"} in{" "}
        {regionData.label},{" "}
      ...
      {listings.length === 0 ? (
        <p className="text-gray-500 text-center py-12">
          No pediatricians in {regionData.label} yet. Check back soon!
        </p>
      ) : (
        <>
